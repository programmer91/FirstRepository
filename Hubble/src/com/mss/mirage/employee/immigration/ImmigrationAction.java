...
/**
 * The <code>ImmigrationAction</code>  class is used for getting  details from
 * <i>Immigration.jsp</i> page.
 * <p>
 * Then it invokes setter methods in <code>ImmigrationVTO</code> class and invokes doImmigrationEdit() method
...
public class ImmigrationAction extends ActionSupport implements ServletRequestAware {
    
    /** The dataProvider is   object reference of ApplicationDataProvider   for the accessing  CONSTANT values  . */
    ...
    /** The immigrationService is used for invoking immigration Service methods.
     * {@link com.mss.mirage.util.ServiceLocator}
     */
    private ImmigrationService immigrationService;
    
    /** The currentImmigration is reference object of the ImmigrationVTO . */
    private ImmigrationVTO currentImmigration;
    ...
    private List immigrationStatusList = new ArrayList();
    
    private List passportCountryList = new ArrayList();
    ...
    public ImmigrationAction() {
    }
    
    
    /**
     * The immigrationPrepare() is used for sending lookUp data  Immigration.jsp page.
     *  @return  The name of the string or resultType by this abstract
     *          pathname, or the empty string if this pathname's name sequence
     *          is empty
     */
    public String immigrationPrepare() {
        resultType = LOGIN;
        ...
        if(httpServletRequest.getSession(false).getAttribute(ApplicationConstants.SESSION_USER_ID) != null){
            ...
            if(AuthorizationManager.getInstance().isAuthorizedUser("PREPARE_IMMIGRATION",userRoleId)){
                try{
                    ...
                    setImmigrationStatusList(hibernateDataProvider
                            .getImmigrationStatus(ApplicationConstants.IMMIGRATION_STATUS_OPTIONS));
                    
                    setPassportCountryList(hibernateDataProvider
    ...
    public String checkAction(){
        ...
        if(httpServletRequest.getSession(false).getAttribute(ApplicationConstants.SESSION_USER_ID) != null){
            userRoleId = Integer.parseInt(httpServletRequest.getSession(false).getAttribute(ApplicationConstants.SESSION_ROLE_ID).toString());
            resultType = "accessFailed";
            if(AuthorizationManager.getInstance().isAuthorizedUser("CHECK_IMMIGRATION",userRoleId)){
                try{
                    immigrationPrepare();
                    if(ServiceLocator.getImmigrationService().checkAction(getEmpId()) > 0){
                        setCurrentImmigration(ServiceLocator.getImmigrationService().getImmigration(this.getEmpId()));
                        actionType="editImmigration";
                    }else{
                        actionType="addImmigration";
                    }
                    resultType = SUCCESS;
                }catch (Exception ex){
                    //List errorMsgList = ExceptionToListUtility.errorMessages(ex);
    ...
    /**
     * The doImmigrationAdd() is used for insert the data  from  the immigration.jsp page into the databasetable.
     *  @return  The name of the string or resultType by this abstract
     *          pathname, or the empty string if this pathname's name sequence
     *          is empty
     */
    public String doImmigrationAdd() {
        ...
        if(httpServletRequest.getSession(false).getAttribute(ApplicationConstants.SESSION_USER_ID) != null){
            ...
            userRoleId = Integer.parseInt(httpServletRequest.getSession(false).getAttribute(ApplicationConstants.SESSION_ROLE_ID).toString());
            resultType = "accessFailed";
            if(AuthorizationManager.getInstance().isAuthorizedUser("DO_ADD_IMMIGRATION",userRoleId)){
                try{
                    boolean insertedRows= ServiceLocator.getImmigrationService().addImmigration(this);
                    if(insertedRows){
                        setCurrentImmigration(ServiceLocator.getImmigrationService().getImmigration(this.getEmpId()));
                        actionType="editImmigration"; 
                        resultType = SUCCESS;
                        resultMessage = "Immigration Details has been successfully saved!";
                    }else{
                        resultType = INPUT;
    ...
    /**
     * The doimmigrationEdit() is used for update the data  from  the immigration.jsp page and save update data into the databasetable.
     *  @return  The name of the string or resultType by this abstract
     *          pathname, or the empty string if this pathname's name sequence
     *          is empty
     */
    public String doImmigrationEdit(){
        resultType = LOGIN;
        ...
        if(httpServletRequest.getSession(false).getAttribute(ApplicationConstants.SESSION_USER_ID) != null){
            ...
            userRoleId = Integer.parseInt(httpServletRequest.getSession(false).getAttribute(ApplicationConstants.SESSION_ROLE_ID).toString());
            resultType = "accessFailed";
            if(AuthorizationManager.getInstance().isAuthorizedUser("DO_EDIT_IMMIGRATION",userRoleId)){
                try{
                    ...
                    boolean isEdit=ServiceLocator.getImmigrationService().editImmigration(this);
                    if(isEdit){
                        setCurrentImmigration(ServiceLocator.getImmigrationService().getImmigration(this.getEmpId()));
                        actionType="editImmigration"; 
                        resultType = SUCCESS;
                        resultMessage = "Immigration Details has been updated successfully!";
                    }else{
                        resultType = INPUT;
                        resultMessage = "Sorry! Please Try again!";
                    }
    ...
    public String getImmigration(){
        ...
        if(httpServletRequest.getSession(false).getAttribute(ApplicationConstants.SESSION_USER_ID) != null){
            ...
            userRoleId = Integer.parseInt(httpServletRequest.getSession(false).getAttribute(ApplicationConstants.SESSION_ROLE_ID).toString());
            resultType = "accessFailed";
            if(AuthorizationManager.getInstance().isAuthorizedUser("GET_IMMIGRATION",userRoleId)){
                try{
                    setCurrentImmigration(ServiceLocator.getImmigrationService().getImmigration(this.getEmpId()));
                    immigrationPrepare();
                    resultType = SUCCESS;
                    setActionType("editImmigration");
    ...
    /** Handles the  <code>setCurrentImmigration</code> method.
     * @param request currentImmigration
     */
    public ImmigrationVTO getCurrentImmigration() {
        return currentImmigration;
    }
    
    public void setCurrentImmigration(ImmigrationVTO currentImmigration) {
        this.currentImmigration = currentImmigration;
    }
  ...
  */
    ...
    /* * Returns a List that can then be used for the retreiving the data.
     * <p>
     * This method always returns immediately, whether or not the
     * string exists. When this "edit button " is clicked to edit the page ,
     * the data will be loaded.
     * @return      the immigrationStatusList at the specified URL
     * @see         immigrationStatusList
     */
    public List getImmigrationStatusList() {
        return immigrationStatusList;
    }
    
    /** Handles the  <code>setImmigrationStatusList</code> method.
     * @param request immigrationStatusList
     */
    public void setImmigrationStatusList(List immigrationStatusList) {
        this.immigrationStatusList = immigrationStatusList;
    }
    
    public List getPassportCountryList() {
        return passportCountryList;
    }
    
    public void setPassportCountryList(List passportCountryList) {
        this.passportCountryList = passportCountryList;
    }
