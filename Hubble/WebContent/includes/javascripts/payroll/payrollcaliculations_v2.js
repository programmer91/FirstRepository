...
function refreshPayroll(){
	...
	//alert(temv_v_variablePay)
    document.getElementById("variablePay").value = temv_v_variablePay;
    
    var newGrossPay =  parseFloat(temv_v_grossPay) + parseFloat(temv_v_variablePay);
 //   alert(newGrossPay)
     document.getElementById("newGrossPay").value = parseFloat(newGrossPay).toFixed(2);
    ...
    // removing esi calculations in gross
...
function changeEarnedValues(eve)
{
    ...
    var temv_employeePf="0.0";
    var temv_employerPf="0.0";
 ...
 //   alert(v_daysCount);
    ...
    temv_employeePf=Math.round((parseFloat(v_employeePf)/parseFloat(v_daysInMonths))*parseFloat(v_daysCount));
    temv_employerPf=Math.round((parseFloat(v_employerPf)/parseFloat(v_daysInMonths))*parseFloat(v_daysCount));
    ...
    var temv_earnedEmployeresi=Math.round((parseFloat(v_earnedEmployeresi)/parseFloat(v_daysInMonths))*parseFloat(v_daysCount));
    var temv_earnedEmployeeesi=Math.round((parseFloat(v_earnedEmployeeesi)/parseFloat(v_daysInMonths))*parseFloat(v_daysCount));
    
    temv_EarnedGrossPay = parseFloat(temv_EarnedGrossPay) - (parseFloat(temv_earnedEmployeresi)+parseFloat(temv_earnedEmployeeesi));
    ...
    temv_EarnedNetPaid = (parseFloat(temv_EarnedVariablePay)+parseFloat(temv_EarnedGrossPay)+parseFloat(temv_employerPf))-(parseFloat(temv_employeePf)+parseFloat(temv_EarnedLongtermBonus)+parseFloat(v_ded_Others)+parseFloat(v_ProfessionalTax)+parseFloat(temv_EarnedLife)+parseFloat(temv_EarnedHealth)+parseFloat(v_employerPf));
   ...
   var result = false;
      ...
      x0p('Based on the vacations entered earned values will be changed.. do you want to continue ?', '', 'warning',
     function(button) {
    ...
    if(result)
    {
        ...
        document.getElementById("employeePfActualDetails").value = parseFloat(temv_employeePf).toFixed(2);
        document.getElementById("earnedEmployerPf").value = parseFloat(temv_employerPf).toFixed(2);
        
        document.getElementById("earnedEmployeresi").value = parseFloat(temv_earnedEmployeresi).toFixed(2);
        document.getElementById("earnedEmployeeesi").value = parseFloat(temv_earnedEmployeeesi).toFixed(2);
...
function computeESIContributions(){
    ...
    document.getElementById("employeresi").value = parseFloat(Math.round(v_EmployerShare)).toFixed(2);
    document.getElementById("employeeesi").value = parseFloat(Math.round(v_EmployeeShare)).toFixed(2);
...
function checkTotalDays(eve){
    ...
    // calculating new values
     var daysVacationOriginal =  document.getElementById('daysVacationHidden').value;
        ...
        if(daysVacationChanged<daysVacationOriginal){
            document.getElementById('daysWorked').value=parseInt(daysWorked)+parseInt(daysVactaion); 
        }
       else if(daysVacationChanged>daysVacationOriginal){
            document.getElementById('daysWorked').value=parseInt(daysWorked)-parseInt(daysVactaion); 
        }
        else{
            document.getElementById('daysWorked').value= document.getElementById('daysWorkedHidden').value;
        ...
        daysHolidays = document.getElementById('daysHolidays').value;
        daysWeekends = document.getElementById('daysWeekends').value;
...
function changeEarnedValuesBasedOnDaysWorked(eve){
  ...
  var result = false;
      ...
      x0p('Days worked in this month is changed.. do you want to continue?', '', 'warning',
     function(button) {
...
function calculateNetPaid(val){
    
    var tdsHidden=document.getElementById("tdsDeductionHidden").value;
    var netPaid=document.getElementById("netPaidActualDetails").value;
    var tdsVal=parseFloat(tdsHidden)-parseFloat(val);
   
        netPaid=parseFloat(netPaid)+parseFloat(tdsVal);;
   // alert(netPaid);
   if(window.confirm("NetPaid will be change.Do you want to continue?")){
    document.getElementById("netPaidActualDetails").value=parseFloat(netPaid).toFixed(2);
    
   }else{
       document.getElementById("tdsDeduction").value=tdsHidden;
   }
}